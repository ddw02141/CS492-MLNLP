...
def encode_text(sentences, vectorizer=None, max_len=None, msg_prefix="\n", verbose=True):
    ...
    for s in sentences:
        ...
        for token in tokens:
        	...
        	encoded_sentence.append(vocab.get(token) + 1)
    ...
    for idx, es in enumerate(encoded_sentences):
        ...
        # print(length)
        # print(max_len)
        # print(pad_encoded_sentences.shape)
        # print(pad_encoded_sentences[idx, :length].shape)
        # print("*******************")

        pad_encoded_sentences[idx, :length] = es
...
def build_model(learning_rate,
    ...
    # Maybe typo.. [num_batches, num_hidden] -> [num_batches, num_classes]
    ...
    # logits = tf.Variable(tf.random_normal([512, num_classes]))
    ...
    for epoch in trange(total_epoch):
        ...
        for batch_xs, batch_ys in _batch_generator(train_xs, train_ys, batch_size):
            ...
            # raise NotImplementedError
             _, train_loss = session.run([optimizer, cost], feed_dict={X: batch_xs, Y: batch_ys, keep_prob: keep_prob_value})
